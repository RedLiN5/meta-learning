...
class abstract_optimizer():
    ...
    def _get_init_param(self, param_type, upper_bound, lower_bound, n):
    ...
    def _get_performance(self, param_name, param_list, fix_params=None):
        if fix_params:
            pass
        else:
            fix_params = {}
    ...
    def _get_str_param(self, param_name, param_list, fix_params=None):
        perform_scores = []
        if fix_params:
            pass
        else:
        ...
        if len(self.param_types) > 0:
            for i in range(len(self.param_types)):
                ...
                if param_type != 'str':
                    ...
                    best_param, best_ei = self._get_opt_param(param_type=param_type,
                    ...
                    opt_param = self._get_str_param(param_name=param_name,
    ...
    def _get_opt_param(self, param_type, param_name, param_bound, max_iter):
        if param_type == 'str':
            pass
        else:
            ...
            init_param = self._get_init_param(param_type=param_type,
                                              upper_bound=upper_bound,
                                              lower_bound=lower_bound,
                                              n=100)
            x, y = self._get_performance(param_name=param_name,
                                         param_list=init_param)
            candidate, best = self._get_param(x, y)
            if len(x) < 30:
                ...
                while count <= max_iter:
                    ...
                    try:
                        new_param = self._get_init_param(param_type=param_type,
